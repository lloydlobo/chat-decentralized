<script>
  ...
  onMount(() => {
    ...
    //  map loops ovel all chat msgs & once() to only read a msg once
    //  since msg is immutable we can use on( ) method instead of once()
    // for different chat rooms assign different names to each node
    // Single giant group chat node
    db.get("chat")
      .map()
      .once(async (data, id) => {
        if (data) {
          ...
          var message = {
            ...
            when: GUN.state.arguments(data, "what"), // gets internal timestamp of the property 'what'
          };
          if (message.what) {
            messages = [...messages.slice(-100), message]; // render loop message with {#each} block
          }
        }
      });
  });
